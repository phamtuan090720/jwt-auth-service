...
export const loginHandler = async (
  ...
  try {
    ...
    // Check if user has a valid sessionx
    // const session = await redisClient.get(JSON.stringify(user._id));
    // if (session) {
    //   return next(new AppError('User Logged!', 401));
    // }
    ...
    res.status(200).json({
      status: 'success',
      access_token,
      expiresIn: new Date(Date.now() + config.accessTokenExpiresIn * 60 * 1000).getTime()
    });
  } catch (err: any) {
    next(err);
  }
